...
class GetAllUsersCase(TestCase):
    def setUp(self):
        global c
        c = Client()

    def test_return_empty_array_if_no_users_exist(self):
    ...
    def test_return_all_users(self):
        ...
        self.assertEqual(data[0]["id"], f"{user.id}")
        self.assertEqual(data[0]["type"], "user")
        self.assertEqual(data[0]["attributes"]["first_name"], user.first_name)
        self.assertEqual(data[0]["attributes"]["last_name"], user.last_name)
...
class GetFlightsCase(TestCase):
    ...
    def test_return_user_flights(self):
        ...
        self.assertEqual(attributes["day_hours"], f"{flight_1.day_hours}")
        self.assertEqual(attributes["night_hours"], f"{flight_1.night_hours}")
...
class FlightCreationTestCase(TestCase):
    def test_flight_creation(self):
        ...
        response = self.client.post(
            f"/api/v1/users/{user.id}/flights/", dict(flight_data)
        )
